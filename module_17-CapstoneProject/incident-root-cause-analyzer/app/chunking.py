...
Splitting happens in two phases:
  1. Atomize - recursively break text on the "biggest" separator first
     ...
     pieces still too big - keeps sentences/paragraphs intact wherever
     possible while ensuring every piece fits within chunk_size.
  2. Merge - greedily pack consecutive small pieces back together up to
     chunk_size, so it doesn't end up with one chunk per sentence.

Overlap then carries the tail of each chunk into the start of the next,
